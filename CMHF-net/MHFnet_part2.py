...
class BasicConv(tf.keras.layers.Layer):
    def __init__(self, out_channels, kernel_size, stride=1, padding='same', dilation=1, groups=1, use_bn=True, use_relu=True):
        super(BasicConv, self).__init__()
        self.conv = layers.Conv2D(out_channels, kernel_size, strides=stride, padding=padding, dilation_rate=dilation, groups=groups, use_bias=False)
        self.bn = layers.BatchNormalization() if use_bn else None
        self.relu = layers.ReLU() if use_relu else None
...
class ChannelGate(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        gate_channels = input_shape[-1]
        self.fc1 = layers.Dense(gate_channels // self.reduction_ratio, activation='relu')
        self.fc2 = layers.Dense(gate_channels)
    ...
    def call(self, x):
        ...
        for pool_type in self.pool_types:
            if pool_type == 'avg':
                ...
                channel_att_raw = self.fc2(self.fc1(avg_pool))
            elif pool_type == 'max':
                max_pool = tf.reduce_max(x, axis=[1, 2], keepdims=True)
                channel_att_raw = self.fc2(self.fc1(max_pool))
...
class SpatialGate(tf.keras.layers.Layer):
    def __init__(self):
        ...
        self.compress = lambda x: tf.concat([tf.reduce_max(x, axis=-1, keepdims=True), tf.reduce_mean(x, axis=-1, keepdims=True)], axis=-1)
        self.spatial = BasicConv(1, kernel_size=7, stride=1, padding='same', use_bn=False, use_relu=False)
...
def HSInet(Y,Z, iniUp3x3,iniA,upRank,outDim,HSInetL,subnetL,ratio=32):
    ...
    cbam1 = CBAM(gate_channels=outDim, reduction_ratio=16, pool_types=['avg', 'max'], no_spatial=True)
...
# 
    ...
    cbam2 = CBAM(upRank, no_spatial=True)
    HY = cbam2(HY)
#    HY  = resCNNnetPrior(('Pri%s'%(1)),HY, Y,1,upRank,3,subnetL)
    ...
    for j in range(HSInetL-2):
        ...
        G = MyConvALayer2(filter=tranB)(G)
        cbam3 = CBAM(gate_channels=outDim, reduction_ratio=16, pool_types=['avg', 'max'], no_spatial=True)
         
        G = cbam3(G)
        HY  = HY-G
        HY  = resCNNnet(HY,j+2,upRank, subnetL)
        cbam4 = CBAM(upRank, no_spatial=True)
        HY = cbam4(HY)
